...
function makeExamples(opt){
	...
	for(var i = 0; i < examples.length; i++){
		...
		output = (showtitle ? '<h3>How to do it</h3>':'')+(css ? (showtitle ? '<h4>CSS</h4>':'')+'<pre class="prettyprint lang-css">'+deindent(sanitise(css))+'</pre>':'')+(js ? (showtitle ? '<h4>Javascript</h4>':'')+'<pre class="prettyprint lang-js">'+deindent(sanitise(js))+'</pre>':'')+(code && showtitle ? '<h4>HTML</h4>':'')+(code ? '<pre class="prettyprint lang-html">'+deindent(code)+'</pre>':'');

		// Append the 'How to do it' content
		if(opt.order[0] == "how") S(examples[i]).prepend(output+'<h3>Result</h3>');
		else S(examples[i]).append(output);
